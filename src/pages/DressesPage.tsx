...
export default function DressesPage() {
  const [dresses, setDresses] = useState<Dress[]>(sampleDresses);
  ...
  const handleSave = (data: Partial<Dress>) => {
    if (editingDress) {
      setDresses((prev) => prev.map((d) => (d.id === editingDress.id ? { ...d, ...data } : d)));
    } else {
      setDresses((prev) => [...prev, { ...data, id: Date.now().toString(), images: [], createdAt: new Date().toISOString().split("T")[0] } as Dress]);
    }
            ...
            <div key={dress.id} className="glass-card overflow-hidden group hover:border-primary/30 transition-all duration-300">
              ...
              <div className="p-4 space-y-2">
                ...
                <div className="flex items-center justify-between pt-2">
                  <div>
                    <p className="text-primary font-semibold text-sm">{formatDZD(dress.rentalPrice)}</p>
