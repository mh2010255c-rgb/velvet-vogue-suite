...
import { sampleDresses, sampleBookings, sampleCustomers, formatDZD } from "@/lib/store";
import {
  AreaChart,
  Area,
  XAxis,
...
const revenueData = [
  { month: "Jan", revenue: 125000, rentals: 8 },
  { month: "Feb", revenue: 198000, rentals: 12 },
  { month: "Mar", revenue: 165000, rentals: 10 },
  { month: "Apr", revenue: 245000, rentals: 15 },
  { month: "May", revenue: 210000, rentals: 13 },
  { month: "Jun", revenue: 310000, rentals: 18 },
];

const categoryData = [
  { name: "Wedding", count: 12 },
  { name: "Evening", count: 18 },
  { name: "Cocktail", count: 15 },
...
export default function DashboardPage() {
  const total = sampleDresses.length;
  const available = sampleDresses.filter((d) => d.status === "Available").length;
  const reserved = sampleDresses.filter((d) => d.status === "Reserved").length;
  const rented = sampleDresses.filter((d) => d.status === "Rented").length;
  const sold = sampleDresses.filter((d) => d.status === "Sold").length;
      ...
      <div className="space-y-8 animate-fade-in">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Welcome back to your boutique overview
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard icon={DollarSign} label="Monthly Revenue" value={formatDZD(310000)} sub="+18% from last month" />
          <StatCard icon={CalendarClock} label="Upcoming Returns" value={2} sub="This week" />
          <StatCard icon={Users} label="Total Customers" value={sampleCustomers.length} />
